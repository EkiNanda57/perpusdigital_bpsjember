    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        ...
        <div class="space-y-4 text-gray-700">
            ...
            <div>
                ...
                @if($publikasi->status == 'diterima')
                    <p>Admin</p>
                @else
                    <p>{{ $publikasi->user->name ?? 'Tidak diketahui' }}</p>
                @endif
            </div>
            ...
            <div>
                ...
                @if($publikasi->tipe_file === 'link' && filter_var($publikasi->file_path, FILTER_VALIDATE_URL))
                    <a href="{{ $publikasi->file_path }}" target="_blank"
                       class="bg-green-500 text-white px-3 py-2 rounded hover:bg-green-600 transition">
                        Buka Link Publikasi
                    </a>
                @elseif($publikasi->file_path)
        ...
        <!-- Tombol Kembali -->
        <div class="mt-8">
            <a href="{{ route('operator.publikasi') }}"
