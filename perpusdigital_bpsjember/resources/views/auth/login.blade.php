...
<div class="flex items-center justify-center min-h-screen bg-orange-100">
  <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 space-y-6">
    ...
    <p class="text-sm text-orange-500 text-center">Silakan masuk ke akunmu</p>
    ...
    <p class="text-center text-gray-600 text-sm">
      Belum punya akun?
      <a href="{{ route('register') }}" class="text-orange-600 hover:underline">Daftar sekarang</a>
    </p>
  </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Pilih semua input di dalam form login
        ...
        formInputs.forEach(input => {
            ...
            input.addEventListener('keydown', function(event) {
                ...
                if (event.key === 'Enter') {
                    // Mencegah perilaku default lain yang mungkin ada
                    event.preventDefault();
                    // Kirim (submit) form-nya
